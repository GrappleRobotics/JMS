...
export default class EnumToggleGroup extends React.Component {
  render() {
    let { name, disabled, variant, outline, values, value, ...props } = this.props;
    ...
    return <ToggleButtonGroup className="enum-toggle" name={name} type="radio" value={value} {...props}>
      {
        values.map(v => <ToggleButton disabled={disabled} variant={ value == v ? variant_active : variant } value={v}>
          { v }
        </ToggleButton>)
      }
    </ToggleButtonGroup>
