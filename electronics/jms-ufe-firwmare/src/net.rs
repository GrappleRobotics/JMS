use smoltcp::iface::{
  Interface, InterfaceBuilder, Neighbor, NeighborCache, Route, Routes, SocketHandle, SocketStorage,
};
...
use modbus::{ModbusTCPFrameRequest, ModbusTCPFrameResponse, Packable, Unpackable};
...
pub struct NetStorageStatic<'a> {
  ...
  dmx_udp_buf: (
    [UdpPacketMetadata; UDP_META_SIZE],
    [u8; UDP_BUF_SIZE],
    [UdpPacketMetadata; UDP_META_SIZE],
    [u8; UDP_BUF_SIZE],
  ),
...
pub static mut STORE: NetStorageStatic = NetStorageStatic {
  ...
  dmx_udp_buf: (
    [UdpPacketMetadata::EMPTY; UDP_META_SIZE],
    [0; UDP_BUF_SIZE],
    [UdpPacketMetadata::EMPTY; UDP_META_SIZE],
    [0; UDP_BUF_SIZE],
  ),
...
impl<'a> Net<'a> {
  pub fn new(
    ...
    let mut iface = InterfaceBuilder::new(ethdev, &mut store.socket_storage[..])
    ...
    Net {
      modbus_socket_handle: iface.add_socket(modbus_socket),
      dmx_socket_handle: iface.add_socket(dmx_socket),
      iface,
    }
  }
  ...
  pub fn poll(&mut self, now: i64, io: &mut IOStates) {
    ...
    self.iface.poll(timestamp).map(|_| ()).unwrap();
            ...
            let response = match req.function {
              modbus::ModbusFunctionRequest::ReadDiscreteInputs { first_address, n } => {
                modbus::ModbusFunctionResponse::ReadDiscreteInputs(
                  io.digital_in
                    .get(first_address as usize..(first_address + n) as usize)
                    .ok_or(modbus::ModbusExceptionCode::IllegalDataAddress),
                )
              }
              ...
              modbus::ModbusFunctionRequest::WriteCoils {
                first_address,
                n,
                values,
                ...
                match io
                  .digital_out
                  .get_mut(first_address as usize..(first_address + n) as usize)
                {
                  Some(v) => {
                    v.clone_from_slice(&values[0..n as usize]);
                    Ok((first_address, n))
                  }
                  None => Err(modbus::ModbusExceptionCode::IllegalDataAddress),
                },
              ...
              modbus::ModbusFunctionRequest::WriteHoldingRegisters {
                first_address,
                n,
                values,
