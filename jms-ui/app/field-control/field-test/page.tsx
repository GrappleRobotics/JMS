...
export default withPermission(["ManageElectronics"], function FieldTest() {
  ...
  return <Container className="mt-3">
    ...
    <br />
    {
      endpoints.map(ep => <Accordion key={`ep-${ep.mac}`} defaultActiveKey="0" className="mt-3">
        <Accordion.Item eventKey="0">
          <Accordion.Header> <h4>{ ep.mac } - { JSON.stringify(ep.status.role) }</h4> </Accordion.Header>
          <Accordion.Body>
            <Row>
              {
            ...
            <Row className="mt-3">
              <Col>
                <EnumToggleGroup
                  ...
                  onChange={v => call<"electronics/update">("electronics/update", { update: { SetRole: { mac: ep.mac, role: v } } })}
                  variant="secondary"
                />
              </Col>
              <Col md="auto">
                <Button variant="green" onClick={() => call<"electronics/update">("electronics/update", { update: { Blink: { mac: ep.mac } } })}> Blink </Button>
              </Col>
            </Row>
          </Accordion.Body>
        </Accordion.Item>
