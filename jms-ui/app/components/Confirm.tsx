...
type ConfirmModalOptions<T> = {
  ...
  renderInner?: (data: T, onUpdate: (data: T) => void, ok: (data?: T) => void, cancel: () => void) => React.ReactNode,
  data?: T
    ...
    >
      ...
      }
      {
        render ? render(okFn, cancelFn) : <React.Fragment>
          <Modal.Body> 
            {
              renderInner ? renderInner(this.state.data, (data: any) => this.setState({ data: data }), (data) => okFn(data === undefined ? this.state.data : data), cancelFn)
