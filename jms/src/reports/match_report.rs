...
use genpdf::{elements::{PageBreak, TableLayout}, style};

use crate::{db, models, reports::{pdf_table, render_header, report_pdf}};
...
fn render_match_table(matches: &Vec<models::Match>, team_highlight: Option<i32>) -> TableLayout {
  ...
  let rows: Vec<Vec<style::StyledString>> = matches.iter().map(|m| {
    vec![
...
pub fn match_report_per_team(mtype: models::MatchType) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
  ...
  for (i, team) in teams.iter().enumerate() {
    render_header(&mut doc, &format!("{:?} Match Report ({})", mtype, team.id), &event_name);
