...
use crate::{models::{Alliance, Match, MatchSubtype, PlayoffAlliance}, schedule::create_tiebreaker};
...
pub fn round_robin_update(alliances: &Vec<PlayoffAlliance>, existing_matches: &Option<Vec<Match>>) -> GenerationUpdate {
  match existing_matches {
    None => GenerationUpdate::NewMatches(generate_initial(alliances)),
    Some(matches) => {
      if matches.len() == 0 {
        GenerationUpdate::NewMatches(generate_initial(alliances))
        ...
        if matches.iter().any(|m| m.match_subtype.unwrap() == MatchSubtype::Final) {
          ...
          let final_matches: Vec<&Match> = matches.iter().filter(|m| m.match_subtype == Some(MatchSubtype::Final)).collect();
          let red_alliance = alliances.iter().find(|a| a.id == final_matches[0].red_alliance.unwrap()).unwrap();
          let blue_alliance = alliances.iter().find(|a| a.id == final_matches[0].blue_alliance.unwrap()).unwrap();

          let red_wins = final_matches.iter().filter(|m| m.winner == Some(Alliance::Red)).count();
          let blue_wins = final_matches.iter().filter(|m| m.winner == Some(Alliance::Blue)).count();
          ...
          if red_wins >= 2 {
            ...
            GenerationUpdate::NewMatches(vec![create_tiebreaker(red_alliance.id, blue_alliance.id, matches, MatchSubtype::Final)])
          }
        } else {
          // Still in RR Semis
          ...
          if standings[0].1 == standings[1].1 {
            // Need a tiebreaker
            GenerationUpdate::NewMatches(vec![ create_tiebreaker(standings[0].0, standings[1].0, matches, MatchSubtype::Semifinal) ])
          } else if standings[1].1 == standings[2].1 {
            // Need a tiebreaker for 2nd
            GenerationUpdate::NewMatches(vec![ create_tiebreaker(standings[1].0, standings[2].0, matches, MatchSubtype::Semifinal) ])
          } else {
            // Generate finals, nice and easy
            GenerationUpdate::NewMatches(vec![
              IncompleteMatch { red: standings[0].0, blue: standings[1].0, playoff_type: MatchSubtype::Final, set: 1, match_num: 1 },
              IncompleteMatch { red: standings[0].0, blue: standings[1].0, playoff_type: MatchSubtype::Final, set: 1, match_num: 2 },
            ])
          }
        }
      } else {
      ...
      match (a, b) {
        (Some(a), Some(b)) => {
          match_pairings.push(IncompleteMatch {
            ...
            set: round as i32 + 1, match_num: i as i32 + 1
          });
        },
        (a, b) => {
...
mod tests {
  use super::*;
  use crate::models::{SQLJson, MatchType};

  #[test]
  fn test_rr_initial() {
    let alliances = vec![
      PlayoffAlliance { id: 1, teams: SQLJson(vec![]), ready: true },
      PlayoffAlliance { id: 2, teams: SQLJson(vec![]), ready: true },
      PlayoffAlliance { id: 3, teams: SQLJson(vec![]), ready: true },
    ];
  ...
  fn test_rr_tiebreaker_2nd() {
    let alliances = vec![
      ...
      PlayoffAlliance { id: 2, teams: SQLJson(vec![]), ready: true },
      PlayoffAlliance { id: 3, teams: SQLJson(vec![]), ready: true },
      PlayoffAlliance { id: 4, teams: SQLJson(vec![]), ready: true },
    ];
  ...
  fn make_rr_semi_match(set: i32, num: i32, played: bool, blue_alliance: i32, red_alliance: i32, winner: Option<Alliance>) -> Match {
