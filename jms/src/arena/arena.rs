...
use crate::{context, models::Team};
use super::exceptions::{ ArenaResult, ArenaError };
...
impl Arena {
  pub fn new(num_stations_per_alliance: u32) -> Arena {
    let mut a = Arena { 
  ...
  pub fn update(&mut self) -> ArenaResult<()> {
    context!("Arena::Update", {
      ...
      let result = match current_state {
        ArenaState::Idle => {
          if self.has_pending_signal(ArenaSignal::PreStart) && self.change_state_or_log(ArenaState::PreMatch) {
            // State has changed
          } else {
  ...
  pub fn can_change_state(&self, desired: ArenaState) -> ArenaResult<()> {
    let err = move |why: &str| { ArenaError::IllegalStateChange { from: self.state, to: desired, condition: why.to_owned() } };
    ...
    match (self.state, desired) {
      ...
      (ArenaState::MatchPlay, ArenaState::MatchComplete) => {
        ...
        let ref current_match = self.current_match.expect("Match is null");
        if current_match.complete() { Ok(()) } else { Err(err("Match is not over")) }
      },
  ...
  fn change_state(&mut self, desired: ArenaState) -> ArenaResult<()> {
    context!("Arena::ChangeState", {
      ...
      info!("Attempting state transition: {:?} -> {:?}", current, desired);
