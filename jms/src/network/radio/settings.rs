...
impl Interactive for FieldRadioSettings {
  async fn interactive() -> anyhow::Result<Self> {
    ...
    let user = inquire::Text::new("Field Router Username?")
      .with_default("root")
      .prompt()?;
    let password = inquire::Text::new("Field Router Password?")
      .with_default("root")
      .prompt()?;
    ...
    if inquire::Confirm::new("Do you want to setup an admin network?")
      .with_default(true)
      .prompt()?
    {
      cfg.admin_ssid = Some(inquire::Text::new("Admin SSID?").with_default("JMS").prompt()?);
      cfg.admin_key = Some(
        inquire::Text::new("Admin Password?")
          .with_default("jmsR0cks")
          .prompt()?,
      );
    }

    if inquire::Confirm::new("Do you want to manually configure radio channels?")
      .with_default(false)
      .prompt()?
    {
